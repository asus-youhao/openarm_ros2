...
def _parse_args():
    ...
    p.add_argument("--lpf-alpha", default="0.3,0.3,0.3,0.3,0.6,0.6,0.6",
                   dest="lpf_alpha",
                   help="Output-side 1st-order LPF on joint cmd. "
                        "'1.0' = off.  Single value '0.5' = uniform; "
    ...
    p.add_argument("--ori-lpf-alpha", type=float, default=0.35,
                   dest="ori_lpf_alpha",
                   help="Input-side SLERP EMA alpha for target orientation "
                        "(default: 0.35; 1.0 disables)")
    p.add_argument("--joint-jump-guard-deg", type=float, default=15.0,
                   dest="joint_jump_guard_deg",
                   help="Reject IK output if any joint changes more than this "
                        "many degrees in one step (default: 15; <=0 disables)")
    p.add_argument("--ee-delta-gap-sec", type=float, default=0.8,
                   dest="ee_delta_gap_sec",
                   help="Tracker message gap before resetting ee_delta reference "
                        "(default: 0.8)")
    ...
    p.add_argument("--no-rot-tracking", action="store_true",
                   ...
                   help="Position-only IK; keeps orientation task disabled")
