...
"""
tracker_ee_delta_ik_controller_tf.py — TF2 version for incremental control
===========================================================================
★ NEW VERSION with TF2 support — Use current EE position instead of home ★

This is an improved version of tracker_ee_delta_ik_controller.py that:
  - Uses TF2 to get current end-effector position from joint7
  - Applies delta from CURRENT position instead of HOME position
  - Enables incremental control from ANY robot pose (no jump to home!)

Key differences from original version:
  OLD: target = home + delta           → robot jumps to home when SPACE pressed ❌
  NEW: target = current_ee + delta     → robot moves from current position ✅
...
架構：
    ...
    鍵盤模擬器（現在，--pattern keyboard）
         │
         ▼
    tracker_ee_delta_ik_controller_tf.py  [THIS FILE - TF2 VERSION]
         │  TF2: get current_ee from joint7
         │  target = current_ee + delta (incremental!)
...
📌 支援的 Input 模式（--pattern）：
    ...
    信號鏈：
        tracker_ee_delta.py ──► /ee_delta/{tracker-side} ──► THIS node
            **[NEW]** Uses TF2 to get current EE pose from joint7
            arm_target_xyz  = current_ee_xyz + R_calib * delta_xyz  (incremental!)
            arm_target_quat = current_ee_quat * delta_quat          (if --rot-tracking)
            → /compute_ik → JointTrajectory → arm controller

    重要改進 (v2.0): **从当前EE位置开始增量控制**
        - 旧版：arm_target = home + delta  → 按 SPACE 时机器人跳回 home 位置 ❌
        - 新版：arm_target = current_ee + delta  → 机器人从当前位置开始移动 ✅
        - 使用 TF2 实时查询 {arm}_link7 当前位姿（base_link → link7）
        - 无需事先移动到 home，任意姿态按 SPACE 都能立即开始控制
...
_ARM_CONFIG = {
    ...
    "right": {
        ...
        # EE at all-joints-zero: (0, -0.153, 0.262) — mirror of left (0, +0.153, 0.262)
        # qx=1 (180° around X) = arm-down orientation
        "home_pose":     (0.0, -0.153, 0.262, 1.0, 0.0, 0.0, 0.0),
        # Workspace: right side (Y negative), expanded to include arm-down range
...
class PicoDeltaIKController(Node):
    ...
    def __init__(self, args):
        ...
        self.get_logger().info(f"TF2 listener initialized for {cfg['ee_link']} → {cfg['base_link']}")
        ...
        # Captured ONCE per button-press session.
        # target = ref_arm + tracker_delta  →  no drift, starts from current arm pose.
        self._ee_delta_ref_xyz   = None   # arm EE xyz captured at session start
        self._ee_delta_ref_q     = None   # arm EE quat captured at session start
        self._ee_delta_last_t    = None   # time.time() of last received ee_delta msg
        # Gap threshold: if no message for this many seconds, next msg = new session.
        # Must be > tracker dead-zone latency but << typical button hold duration.
    ...
    def get_current_ee_from_tf(self, timeout_sec=0.5):
        """
        Get current EE pose from TF (base_link → ee_link).
        Returns (x, y, z, qx, qy, qz, qw) or None if fails.
        Uses blocking lookup with timeout.
        """
        try:
            # Lookup transform with timeout
            transform = self._tf_buffer.lookup_transform(
                self.cfg["base_link"],  # target frame (world)
                self.cfg["ee_link"],     # source frame (joint7)
            ...
            # Return as 7-tuple: (x, y, z, qx, qy, qz, qw)
    ...
    def set_target_and_send(self, delta_xyz, delta_quat):
        """
        Set arm EE target = current_ee + R_calib*delta_xyz, current_quat*delta_quat.
        Uses TF to get current EE pose (NO drift from home position).
        Replaces self._pose directly (no accumulation → no drift).
        Returns (success, ik_ms).
        """
        ws = self.cfg["workspace"]

        # ── Use the session reference pose (captured once at session start) ───
        # Tracker sends TOTAL delta from its reference (not per-frame):
        #   target = ref_arm + delta  →  stable, no drift
        # Reading TF every frame would cause: target = TF_moving + delta → drift.
        if self._ee_delta_ref_xyz is not None:
            current_xyz = self._ee_delta_ref_xyz
            current_q   = self._ee_delta_ref_q
        else:
            # Fallback: session not yet established → use last commanded position.
            # Never use home_pose: arm may already be far from home.
        ...
        # Target = current + delta (incremental control)
        ...
        if getattr(self.args, 'no_rot_tracking', False):
            new_q = current_q  # Keep current orientation
        else:
            # Rotation: pre-multiply delta_quat (WORLD frame rotation).
            # tracker delta_quat = q_curr * q_ref^-1 → applies in world frame.
            # Correct:  target_q = delta_quat * ref_q   (pre-multiply)
            # Wrong was: target_q = ref_q * delta_quat  (body-frame, caused RPY drift)
    ...
    def run_pattern_ee_delta(self):
        ...
        while True:
            ...
            if msg is not None and isinstance(msg, PoseStamped):
                ...
                # ── Session detection via TIME GAP ────────────────────────────
                # tracker_ee_delta has a dead zone: near-zero deltas are NOT
                # published. So the IK controller never sees "delta near 0" as
                # a session boundary. Instead, detect the silence between button
                # releases: if no message for >gap_sec → this msg is a new session.
                ...
                if is_new_session:
                    # Capture current arm EE from TF as the reference for this session.
                    # target = ref_arm + tracker_delta → arm moves from where it IS now.
                    ...
                    if ref is not None:
                        ...
                        print(f"\n  [EE-δ] NEW SESSION (gap={gap:.2f}s) | "
                              f"arm_ref=[{ref[0]:.3f},{ref[1]:.3f},{ref[2]:.3f}] "
                              f"(from TF)")
                    else:
                        # TF failed → fall back to last successfully commanded pose.
                        # Do NOT use home_pose: arm may be far from home.
                        ...
                        print(f"\n  [EE-δ] NEW SESSION (gap={gap:.2f}s) | "
                              ...
                              f"(TF FAILED → using last commanded pose)")
