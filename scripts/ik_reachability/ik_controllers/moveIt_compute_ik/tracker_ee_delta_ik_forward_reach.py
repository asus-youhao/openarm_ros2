...
"""
tracker_ee_delta_ik_forward_reach.py — ForwardReachIK version (forward-reach-only, no back-scratch)
====================================================================================================
★ v1.0 — Integrates ForwardReachIKSolver to enforce forward-reach-only arm poses ★

Based on tracker_ee_delta_ik_natural.py (NaturalIK elbow-down version).
Added: Stricter joint constraints: j1 limited to forward swing only,
       j3 strictly ≤ 0 (no elbow-up / back-scratching motions).

Changes from tracker_ee_delta_ik_forward_reach.py:
  + Import ForwardReachIKSolver (from forward_reach_ik_solver.py, same directory)
  + self._natural_ik = ForwardReachIKSolver(arm, node=self)
  + _set_ik_seed() helper so ForwardReachIKSolver can swap seeds before retrying
  ...
  + --no-forward-ik flag    → disable (falls back to original single-seed behaviour)
  + --natural-verbose flag  → print each retry attempt score (debug)
  + ForwardReachIK stats printed in final summary

Why forward-reach-only:
  In VR teleop the 7-DOF null-space can produce back-scratching (j3 > 0)
  or backward arm swing (j1 < 0) solutions that are unsafe or uncomfortable.
  ForwardReachIKSolver restricts j1 to [-0.50, +1.30] rad and j3 to ≤ 0 rad
  so only natural forward-facing reach poses are accepted.

How ForwardReachIKSolver works:
  1. First tries current seed (fast path, ~2 ms).
  2. If result violates forward-reach constraints, retries with seeds
     biased toward j3 < 0 (elbow-down) and j1 in forward range.
  3. Returns the candidate with lowest "naturalness cost".
  Avg overhead: < 8 ms when first-try succeeds.
...
Dependencies:
    forward_reach_ik_solver.py must be in the same directory as this script.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Commands:

  # 右臂 VR tracker 控制（最常用，從當前位置開始）
  python3 tracker_ee_delta_ik_forward_reach.py --arm right --pattern ee_delta

  # 右臂 VR tracker，指定 tracker topic right side
  python3 tracker_ee_delta_ik_forward_reach.py --arm right --pattern ee_delta --tracker-side right

  # 左臂 VR tracker 控制
  python3 tracker_ee_delta_ik_forward_reach.py --arm left --pattern ee_delta

  # 雙臂同時啟動（各開一個 Terminal）
  python3 tracker_ee_delta_ik_forward_reach.py --arm right --pattern ee_delta &
  python3 tracker_ee_delta_ik_forward_reach.py --arm left  --pattern ee_delta

  # 鍵盤測試（確認 ForwardReachIK 不影響動作）
  python3 tracker_ee_delta_ik_forward_reach.py --arm right --pattern keyboard

  # 鍵盤測試 + 顯示每次 IK 嘗試分數（除錯 naturalness）
  python3 tracker_ee_delta_ik_forward_reach.py --arm right --pattern keyboard --natural-verbose

  # 正弦波自動測試（驗證肘部不會彈起）
  python3 tracker_ee_delta_ik_forward_reach.py --arm right --pattern sine --duration 30

  # 圓型軌跡測試
  python3 tracker_ee_delta_ik_forward_reach.py --arm right --pattern circle --radius 0.06

  # 先回 home 再開始
  python3 tracker_ee_delta_ik_forward_reach.py --arm right --pattern ee_delta --home-first

  # 退出時回 home
  python3 tracker_ee_delta_ik_forward_reach.py --arm right --pattern ee_delta --home-on-exit

  # 停用 ForwardReachIK（退回原始單 seed 行為，效能對比用）
  python3 tracker_ee_delta_ik_forward_reach.py --arm right --pattern keyboard --no-forward-ik

  # 快速模式（停用碰撞，speed > safety）
  python3 tracker_ee_delta_ik_forward_reach.py --arm right --pattern sine --no-collisions

  # 存計時 CSV + 自動生成 PNG 圖表
  python3 tracker_ee_delta_ik_forward_reach.py --arm right --pattern circle --timing-csv /tmp/natural_ik.csv

  # Dry run（只算 IK，不送指令到手臂，安全測試）
  python3 tracker_ee_delta_ik_forward_reach.py --arm right --pattern sine --dry-run
...
_ARM_CONFIG = {
    ...
    "right": {
        ...
        "default_quat":  (0.0, 0.0, 0.707, 0.707),
        "home_pose":     (0.0, -0.25, 0.45, 0.0, 0.0, 0.707, 0.707),
...
class PicoDeltaIKController(Node):
    ...
    def __init__(self, args):
        super().__init__("pico_forward_reach_ik_controller")
        self.args = args
        ...
        # ── ForwardReachIKSolver: multi-seed IK with forward-reach-only enforcement ──
        # Disabled when --no-forward-ik is passed.
    ...
    # ------------------------------------------------------------------
    # ForwardReachIK seed setter
    # ------------------------------------------------------------------
    def _set_ik_seed(self, joints: list):
        """
        Helper used by ForwardReachIKSolver.call_natural() to swap the seed
    ...
    def set_target_and_send(self, delta_xyz, delta_quat):
        ...
        t_total = time.time()
        ok, joints, ik_ms = self._call_ik()   # ← ForwardReachIK here
        if ok:
            with self._joints_lock:
    ...
    def run_pattern_keyboard(self):
        ...
        # 先用 TF2 取得實際末端姿態，再用 /joint_states 作為 IK seed
        print("  同步 EE 姿態從 TF2...")
    ...
    def _plot_results(self, csv_path: pathlib.Path):
        ...
        natural_tag = "ForwardReachIK=ON" if self._natural_ik else "ForwardReachIK=OFF"
        fig.suptitle(
            f"ForwardReachIK — {self.args.arm} arm  [{self.args.pattern}]  {natural_tag}  "
            f"n={len(ik_times)}  "
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Pico VR delta IK controller — ForwardReachIK version (forward-reach-only)",
...
def main():
    ...
    print(f"\n  Pico Delta IK Controller — ForwardReachIK={natural_tag}")
        ...
        if records:
            ...
            if node._natural_ik is not None:
                ...
                if total > 0 and first_pct < 80:
                    ...
                    print(f"      in forward_reach_ik_solver.py _RIGHT_CFG / _LEFT_CFG")
            print(f"{'='*60}")
