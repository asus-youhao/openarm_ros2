#!/usr/bin/env python3
"""
pico_delta_ik_controller.py — Pico VR 搖桿相對運動 IK 控制器
==============================================================
...
📌 Pico VR 橋接說明（未來）：
    在 Pico 裝置上執行 ROS2 橋接節點，發布：
        /pico/delta_twist  geometry_msgs/TwistStamped
            linear.x/y/z  : 手柄相對位移 (m/s × dt)
            angular.x/y/z : roll/pitch/yaw 增量 (rad/s × dt)
    本節點訂閱後即可直接控制。
...
_ARM_CONFIG = {
    "left": {
        ...
        "group_name":    "left_arm",
        # IK-solved home: (0.0, +0.25, 0.45) qx=1, mirror of right arm
        # Verified by /compute_ik probe: joints differ from right (not simple negation)
        "home_joints":   [0.8357, -1.066, 0.4933, 1.8287, -1.0751, 0.4608, 0.4202],
        "default_quat":  (1.0, 0.0, 0.0, 0.0),
        "home_pose":     (0.0, 0.25, 0.45, 1.0, 0.0, 0.0, 0.0),
        # Left arm workspace is Y-mirror of right arm
        "workspace":     {"x": (-0.20, 0.25), "y": (0.10, 0.45), "z": (0.25, 0.65)},
...
# ---------------------------------------------------------------------------
# Named home positions (--home 1..5)
# pose = (x, y, z, qx, qy, qz, qw)   — all in world frame, orientation qx=1 (arm-down)
# joints = [j1..j7]                   — IK-solved seed for each arm
# ---------------------------------------------------------------------------
_HOME_POSES = {
    # right arm positions              left arm positions (Y-mirrored)
    1: {
        "name":   "center",
        "right":  {"pose": (0.00, -0.25, 0.45, 1.0,0.0,0.0,0.0),
                   "joints": [0.0, 1.2735, -1.5708, 1.8287, 1.5708, -0.5552, 0.0]},
        "left":   {"pose": (0.00,  0.25, 0.45, 1.0,0.0,0.0,0.0),
                   "joints": [0.8357,-1.066,0.4933,1.8287,-1.0751,0.4608,0.4202]},
    },
    2: {
        "name":   "forward_low",
        "right":  {"pose": (0.20, -0.25, 0.32, 1.0,0.0,0.0,0.0),
                   "joints": None},   # computed at runtime on first use
        "left":   {"pose": (0.20,  0.25, 0.32, 1.0,0.0,0.0,0.0),
                   "joints": None},
    },
    3: {
        "name":   "side_extend",
        "right":  {"pose": (0.00, -0.38, 0.40, 1.0,0.0,0.0,0.0),
                   "joints": None},
        "left":   {"pose": (0.00,  0.38, 0.40, 1.0,0.0,0.0,0.0),
                   "joints": None},
    },
    4: {
        "name":   "high_raise",
        "right":  {"pose": (0.05, -0.22, 0.60, 1.0,0.0,0.0,0.0),
                   "joints": None},
        "left":   {"pose": (0.05,  0.22, 0.60, 1.0,0.0,0.0,0.0),
                   "joints": None},
    },
    5: {
        "name":   "table_reach",
        "right":  {"pose": (0.22, -0.28, 0.30, 1.0,0.0,0.0,0.0),
                   "joints": None},
        "left":   {"pose": (0.22,  0.28, 0.30, 1.0,0.0,0.0,0.0),
                   "joints": None},
    },
}
...
class PicoDeltaIKController(Node):
    ...
    def __init__(self, args):
        ...
        self._pending_delta = None    # latest TwistStamped from Pico
    ...
    def go_to_named_home(self, home_id: int):
        """
        Move arm to one of the 5 named home positions (--home 1..5).
        If joints for this pose are None (not yet IK-solved), solve them first.
        """
        entry = _HOME_POSES.get(home_id)
        if entry is None:
            self.get_logger().error(f"Invalid home id {home_id} — use 1..5")
            return False

        arm_entry = entry[self.args.arm]
        pose = arm_entry["pose"]
        joints = arm_entry["joints"]

        # Solve IK for this pose if not precomputed
        if joints is None:
            self.get_logger().info(
                f"[Home {home_id}] IK-solving '{entry['name']}' "
                f"at {pose[:3]} for {self.args.arm} arm…")
            # Temporarily set pose and call IK with zero seed
            saved_pose = list(self._pose)
            self._pose = list(pose)
            with self._joints_lock:
                saved_js = list(self._last_joints)
                self._last_joints = list(self.cfg["home_joints"])  # zero seed
            ok, solved_joints, ik_ms = self.call_ik_sync()
            if not ok:
                self._pose = saved_pose
                with self._joints_lock:
                    self._last_joints = saved_js
                self.get_logger().error(
                    f"[Home {home_id}] IK failed for '{entry['name']}' — staying at current pose")
                return False
            # Cache result for future use
            arm_entry["joints"] = solved_joints
            joints = solved_joints
            self.get_logger().info(
                f"[Home {home_id}] IK solved in {ik_ms:.1f}ms → {[round(j,3) for j in joints]}")
        else:
            self.get_logger().info(
                f"[Home {home_id}] Going to '{entry['name']}' {pose[:3]}")
...
def parse_args():
    ...
    p.add_argument("--pattern", default="keyboard",
                   choices=["keyboard", "sine", "circle", "lemniscate", "topic"])
    ...
    p.add_argument("--home", type=int, default=None, metavar="N",
                   choices=[1,2,3,4,5],
                   help="Go to named home position N (1=center, 2=forward_low, "
                        "3=side_extend, 4=high_raise, 5=table_reach) before pattern")
    p.add_argument("--home-first", action="store_true",
                   help="Send arm to home#1 (center) before starting")
    p.add_argument("--home-on-exit", action="store_true",
                   help="Return home on Ctrl-C")
    p.add_argument("--bimanual", action="store_true",
                   help="Run both arms simultaneously (--arm ignored, uses both)")
...
def _run_single_arm(args):
    """Run one arm controller node — used by single-arm mode."""
    ...
    try:
        ...
        # Named home (--home N) takes priority over --home-first
        if args.home is not None:
            entry = _HOME_POSES[args.home]
            print(f"\n  Going to home#{args.home} '{entry['name']}'...")
            node.go_to_named_home(args.home)
            time.sleep(3.5)
        elif args.home_first:
            print("\n  Moving to home#1 (center)...")
            node.go_to_named_home(1)
            time.sleep(3.5)
...
def _print_home_table():
    print(f"\n  Named home positions (--home N):")
    for n, e in _HOME_POSES.items():
        rp = e["right"]["pose"]
        print(f"    {n}: {e['name']:<14}  right=({rp[0]:.2f},{rp[1]:.2f},{rp[2]:.2f})  "
              f"left=mirrored Y")
...
def main():
    ...
    if args.bimanual:
        # Bimanual: run left and right in separate threads sharing the same rclpy context
        import copy
        rclpy.init()

        args_right = copy.copy(args)
        args_right.arm = "right"
        args_right.timing_csv = args.timing_csv.replace(".csv", "_right.csv")

        args_left = copy.copy(args)
        args_left.arm = "left"
        args_left.timing_csv = args.timing_csv.replace(".csv", "_left.csv")

        print("\n  BIMANUAL mode — running both arms simultaneously")
        print("  NOTE: keyboard pattern not supported in bimanual; use sine/circle/topic")
        if args.pattern == "keyboard":
            print("  Switching to sine pattern for bimanual demo.")
            args_right.pattern = "sine"
            args_left.pattern = "sine"

        node_r = PicoDeltaIKController(args_right)
        node_l = PicoDeltaIKController(args_left)
        ...
        node_r._wait_for_ik_service()
        node_l._wait_for_ik_service()
        print("  /compute_ik ready ✓")

        # Go to named home if requested
        home_id = args.home if args.home is not None else (1 if args.home_first else None)
        if home_id is not None:
            print(f"  Going to home#{home_id} for both arms...")
            node_r.go_to_named_home(home_id)
            node_l.go_to_named_home(home_id)
            time.sleep(3.5)

        def run_arm(node):
            try:
                pat = node.args.pattern
                if pat == "sine":        node.run_pattern_sine()
                elif pat == "circle":    node.run_pattern_circle()
                elif pat == "lemniscate":node.run_pattern_lemniscate()
                elif pat == "topic":     node.run_pattern_topic()
            except KeyboardInterrupt:
                pass

        t_r = threading.Thread(target=run_arm, args=(node_r,), daemon=True)
        t_l = threading.Thread(target=run_arm, args=(node_l,), daemon=True)
        try:
            t_r.start(); t_l.start()
            t_r.join();  t_l.join()
        except KeyboardInterrupt:
            print("\n  Bimanual interrupted.")
        finally:
            executor.shutdown()
            node_r.destroy_node(); node_l.destroy_node()
            rclpy.shutdown()
        return

    # Single arm
    rclpy.init()
    _run_single_arm(args)
    rclpy.shutdown()
