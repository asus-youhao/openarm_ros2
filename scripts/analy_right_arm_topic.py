#!/usr/bin/env python3
"""
Topic-based analyzer for OpenArm (left, right, or bimanual).

Monitors:
 - /right_forward_position_controller/commands (or /left_... or both)
 - /joint_states
...
Usage:
  ...
  python3 scripts/analy_right_arm_topic.py
  
  # Left arm only
  python3 scripts/analy_right_arm_topic.py --arm left
  
  # Both arms (bimanual)
  python3 scripts/analy_right_arm_topic.py --arm both
  
  # Custom options
  python3 scripts/analy_right_arm_topic.py --arm right --no-plot --error-scale deg
"""
...
class ArmTopicAnalyzer(Node):
    def __init__(self, arm_config, command_type, joint_names=None, csv_dir='analy_arm', plot=True, error_scale='rad'):
        super().__init__('arm_topic_analyzer')
        self.arm_config = arm_config  # 'right', 'left', or 'both'
        self.command_type = command_type
        self.plot = plot and HAS_MPL
        self.error_scale = error_scale.lower()

        # Build joint names and command topics based on arm_config
        if joint_names is not None and len(joint_names) > 0:
            self.joint_names = joint_names
            self.command_topics = []  # will need manual override
        ...
        self.get_logger().info(f'Arm analyzer: arm={arm_config} type={command_type}')
        ...
        # Track which command topic maps to which joints
        if arm_config == 'both':
            ...
            if self.plot:
                ...
                for idx, name in enumerate(self.joint_names):
                    ...
                    if np.any(~np.isnan(err_plot)):
                        ...
                        err_lines += l_err
                        # filled area for absolute error (visual aid)
                        try:
                ...
                plt.suptitle(f'{self.arm_config.capitalize()} Arm Commands vs States - {timestamp}')
...
def main(argv=None):
    ...
    node = ArmTopicAnalyzer(args.arm, args.command_type, joint_names=args.joint_names,
                           csv_dir=args.csv_dir, plot=args.plot, error_scale=args.error_scale)
