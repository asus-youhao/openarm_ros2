...
bool OpenArm_v10HW::init_kdl_dynamics(const std::string& urdf_content) {
  ...
  // Extract chain for this arm (try to include palm/hand when possible)
  std::string root_link = "openarm_body_link0";

  // Prepare a list of default tip candidates. Prefer the standard link7,
  // but if this hardware has a LEAP hand, also try common palm/fingertip
  // link names (e.g. 'right_palm_lower', 'right_fingertip', etc.).
  std::vector<std::string> default_tips;
  default_tips.push_back(std::string("openarm_") + arm_prefix_ + "link7");
  if (has_leap_hand_) {
    // arm_prefix_ typically "right_" or "left_" for bimanual URDFs,
    // but some URDFs use names without the "openarm_" prefix for hand links.
    // Try both styles to maximize chance of finding the hand links.
    std::string pref_no_openarm = arm_prefix_; // e.g. "right_"
    // remove trailing underscore if present for some URDF naming conventions
    if (!pref_no_openarm.empty() && pref_no_openarm.back() == '_') pref_no_openarm.pop_back();
    RCLCPP_INFO(rclcpp::get_logger("OpenArm_v10HW"),
                "arm_prefix for LEAP hand: '%s'", pref_no_openarm.c_str());

    // Scan URDF for tip links containing keywords and add to default_tips
    std::vector<std::string> urdf_tip_candidates;
    std::string urdf_lower = urdf_content;
    std::transform(urdf_lower.begin(), urdf_lower.end(), urdf_lower.begin(), ::tolower);
    std::vector<std::string> keywords = {"palm", "hand", "finger", "tip"};
    size_t pos = 0;
    while (true) {
      ...
      for (const auto &kw : keywords) {
        if (link_name_lower.find(kw) != std::string::npos) {
          urdf_tip_candidates.push_back(link_name);
          RCLCPP_INFO(rclcpp::get_logger("OpenArm_v10HW"),
                      "Found tip candidate in URDF: '%s'", link_name.c_str());
          break;
        }
      }
      pos = sq + 1;
    }
    // Add found candidates to default_tips (avoid duplicates)
    for (const auto &cand : urdf_tip_candidates) {
      if (std::find(default_tips.begin(), default_tips.end(), cand) == default_tips.end()) {
        default_tips.push_back(cand);
        RCLCPP_INFO(rclcpp::get_logger("OpenArm_v10HW"),
                    "Added URDF tip candidate: '%s' to default_tips", cand.c_str());
      }
    }

    // Common LEAP/O6 link candidates
    default_tips.push_back(pref_no_openarm + std::string("_palm_lower"));
    default_tips.push_back(pref_no_openarm + std::string("_palm"));
    default_tips.push_back(pref_no_openarm + std::string("_fingertip"));
    default_tips.push_back(pref_no_openarm + std::string("_index_tip_head"));
  }

  bool chain_ok = false;
  std::string tip_link;
  // Try the default tip candidates first
  for (const auto &cand_tip : default_tips) {
    if (kdl_tree.getChain(root_link, cand_tip, kdl_chain_)) {
      tip_link = cand_tip;
      chain_ok = true;
      RCLCPP_INFO(rclcpp::get_logger("OpenArm_v10HW"),
                  "Using default candidate tip link '%s' for KDL chain",
                  cand_tip.c_str());
      break;
    }
  }
  ...
  if (!chain_ok) {
    RCLCPP_WARN(rclcpp::get_logger("OpenArm_v10HW"),
                "Default KDL chain candidates from %s not found, attempting to locate hand/palm links in URDF",
                root_link.c_str());
    RCLCPP_WARN(rclcpp::get_logger("OpenArm_v10HW"),
                "Default KDL chain from %s to %s not found, attempting to locate hand/palm links in URDF",
                root_link.c_str(), tip_link.c_str());

    // Fallback: scan URDF content for candidate links (palm/hand/finger) and try them
    std::vector<std::string> candidates;
    std::string urdf_lower = urdf_content;
    // naive lowercase for search
    std::transform(urdf_lower.begin(), urdf_lower.end(), urdf_lower.begin(), ::tolower);

    // Search for likely hand link names within the URDF ("palm", "hand", "finger", "tip")
    std::vector<std::string> keywords = {"palm", "hand", "finger", "tip"};

    // Find all <link name="..."> occurrences and collect candidates matching keywords
    size_t pos = 0;
    while (true) {
      size_t link_pos = urdf_lower.find("<link", pos);
      if (link_pos == std::string::npos) break;
    ...
    // Also try children of link7 by searching joints with parent==link7
    pos = 0;
    while (true) {
      size_t joint_pos = urdf_lower.find("<joint", pos);
      if (joint_pos == std::string::npos) break;
      size_t parent_pos = urdf_lower.find("parent", joint_pos);
      if (parent_pos == std::string::npos) { pos = joint_pos + 6; continue; }
      size_t ln_pos = urdf_lower.find("link=", parent_pos);
      if (ln_pos == std::string::npos) { pos = joint_pos + 6; continue; }
      size_t fq = urdf_lower.find_first_of("\"'", ln_pos + 5);
      if (fq == std::string::npos) { pos = joint_pos + 6; continue; }
      char q = urdf_lower[fq];
      size_t sq = urdf_lower.find(q, fq + 1);
      if (sq == std::string::npos) { pos = joint_pos + 6; continue; }
      std::string parent_name = urdf_content.substr(fq + 1, sq - fq - 1);
      if (parent_name == tip_link) {
        // find child attribute
        size_t child_pos = urdf_lower.find("child", parent_pos);
        if (child_pos == std::string::npos) child_pos = urdf_lower.find("child", joint_pos);
        if (child_pos != std::string::npos) {
          size_t ln_pos2 = urdf_lower.find("link=", child_pos);
          if (ln_pos2 != std::string::npos) {
            size_t fq2 = urdf_lower.find_first_of("\"'", ln_pos2 + 5);
            if (fq2 != std::string::npos) {
              size_t sq2 = urdf_lower.find(urdf_lower[fq2], fq2 + 1);
              if (sq2 != std::string::npos) {
                std::string child_name = urdf_content.substr(fq2 + 1, sq2 - fq2 - 1);
                candidates.push_back(child_name);
              }
            }
          }
        }
    ...
    // Try candidates (prioritize those containing palm/hand)
    ...
    for (const auto &cand : seen) {
      if (kdl_tree.getChain(root_link, cand, kdl_chain_)) {
        RCLCPP_INFO(rclcpp::get_logger("OpenArm_v10HW"),
                    "Using candidate tip link '%s' for KDL chain (was %s)",
                    cand.c_str(), tip_link.c_str());
        tip_link = cand;
        chain_ok = true;
        break;
      }
  ...
  if (!chain_ok) {
    RCLCPP_WARN(rclcpp::get_logger("OpenArm_v10HW"),
                 "Failed to find a suitable KDL chain from %s to %s (including candidates), gravity compensation will be disabled",
                 root_link.c_str(), tip_link.c_str());
    return false;
  }
  ...
  // Resize gravity storage to KDL chain size
